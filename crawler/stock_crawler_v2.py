...
def get_today():
    ...
    timestamp_start = int(datetime.fromisoformat('2021-10-23T00:00:00.000+00:00').timestamp())
    timestamp_end = int(datetime.fromisoformat(today_strftime).timestamp())
    ...
    return timestamp_start, timestamp_end, timestamp_day_before
...
all_list = [computer_stock_list, light_eletric_stock_list, internet_stock_list, component_stock_list, pathway_stock_list,
            it_stock_list, other_electric_stock_list, sail_stock_list, bio_stock_list, finance_stock_list]
# 自動化記得加 stock_market_list

timestamp_start, timestamp_end, timestamp_day_before = get_today()

# for list in all_list:
#     for stock_code in list:
#         try:
#             df = craw_stock(stock_code, timestamp_start, timestamp_end)
#             df_row_insert(df)
#             time.sleep(1)
#         except:
