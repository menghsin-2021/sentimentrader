...
function insertOption(category, stock_code) {
  category = document.getElementById("category");
  stock_code = document.getElementById("stock_code");
  console.log(category.options);
  console.log(stock_code.options);
  if (category.options[category.selectedIndex].value == "electric_car") {
    removeAll(stock_code);
    for (let i=0; i<stock_list_electric_car.length; i++) {
        let opt = document.createElement("option");
        opt.value = stock_list_electric_car[i][0];
        opt.innerHTML = stock_list_electric_car[i][0] +' '+ stock_list_electric_car[i][1];
        stock_code.add(opt);
    }}
   else if (category.options[category.selectedIndex].value == "electric") {
    removeAll(stock_code);
    for (let i=0; i<stock_list_electric.length; i++) {
        let opt = document.createElement("option");
        opt.value = stock_list_electric[i][0];
        opt.innerHTML = stock_list_electric[i][0] +' '+ stock_list_electric[i][1];
        stock_code.add(opt);
  }} else if (category.options[category.selectedIndex].value == "sail") {
  removeAll(stock_code);
    for (let i=0; i<stock_list_sail.length; i++) {
        let opt = document.createElement("option");
        opt.value = stock_list_sail[i][0];
        opt.innerHTML = stock_list_sail[i][0] +' '+ stock_list_sail[i][1];
        stock_code.add(opt);
  }}else if (category.options[category.selectedIndex].value == "biotech") {
  removeAll(stock_code);
    for (let i=0; i<stock_list_biotech.length; i++) {
        let opt = document.createElement("option");
        opt.value = stock_list_biotech[i][0];
        opt.innerHTML = stock_list_biotech[i][0] +' '+ stock_list_biotech[i][1];
        stock_code.add(opt);
  }}else if (category.options[category.selectedIndex].value == "finance") {
  removeAll(stock_code);
    for (let i=0; i<stock_list_finance.length; i++) {
        let opt = document.createElement("option");
...
function showStrategy(strategy_line, custom_strategy) {
  strategy_line = document.getElementById("strategy_line");
...
function strategyStock(strategy_in, strategy_out) {
strategy_in = document.getElementById("strategy_in");
  strategy_out = document.getElementById("strategy_out");
  removeAll(strategy_out)
  if (strategy_in.options[strategy_in.selectedIndex].value == "increase_in") {
    removeAll(strategy_out)
    let opt1 = document.createElement("option");
    opt1.value = 'decrease_out';
    opt1.innerHTML = '股價連續下跌(3日)';
    strategy_out.add(opt1);
    }
   else if (strategy_in.options[strategy_in.selectedIndex].value == "decrease_in") {
    removeAll(strategy_out)
    let opt2 = document.createElement("option");
    opt2.value = 'increase_out';
    opt2.innerHTML = '股價連續上漲(3日)';
    strategy_out.add(opt2);
    } else {
    console.log('error')
  }
...
function strategySentiment(sentiment_condition, sentiment_grade, sentiment_source) {
sentiment_condition = document.getElementById("sentiment_condition");
  sentiment_grade = document.getElementById("sentiment_grade");
